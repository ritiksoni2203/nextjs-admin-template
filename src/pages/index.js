...
// ** Reactstrap Imports
import { Button, Card, CardHeader, CardTitle, Col, Modal, ModalBody, ModalHeader, Row } from "reactstrap";
...
const Clubs = () => {
  ...
  const admin = typeof window !== 'undefined' && localStorage.getItem("isAdmin") === "true" ? true : false;

  useEffect(() => {
    if (reload !== null) {
       dispatch(clubsListApi({ page: currentPage, limit: perPage, search }))
    }
  }, [reload]);
        ...
        <tr key={index}>
          ...
          <td>
            <div className="d-flex align-items-center gap-1">
              <img src={row.image} className="club-image" />
            </div>
          </td>
          <td>{row?.title}</td>
          <td>
      ...
      <Row className="justify-content-between align-items-center mb-2">
        ...
        <Col md='6' className="d-flex justify-content-end w-100">
          <div className='d-flex mt-md-0 mt-1'>
            {!admin && <Button tag={Button} href='/club/addclub' className="btn btn-danger ms-2">Add Club</Button>}
          </div>
        </Col>
      </Row>
