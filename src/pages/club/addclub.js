...
import upload from "../../assets/images/add-club.png";
import uploadDark from "../../assets/images/club-dark.png";
...
const AddClub = () => {
  const skin = typeof window !== 'undefined' && JSON.parse(localStorage.getItem("skin"));
  const { reload, status } = useSelector((store) => ({
      ...
      <Card className="p-3 mt-2">
        <Formik
          ...
          validationSchema={Yup.object().shape({
            ...
            // shaftLength: Yup.number().required("Shaft length is required"),
            // shaftFlex: Yup.string().required("Shaft flex is required"),
          ...
          onSubmit={(values) => {
            ...
            // formData.append("shaftFlex", values.shaftFlex);
            ...
            <Form onSubmit={handleSubmit} onReset={handleReset}>
              <Row>
                <div className="mb-2">
                  ...
                  {errors.clubImage && touched.clubImage ? (
                ...
                {/* <div className='mb-1 w-50'>
                                    ...
                                    <InputGroup>
                                        ...
                                        <Field className="form-control" id='range' placeholder='Range' name="clubRange"
                                            onChange={(e) => setFieldValue("clubRange", e.target.value)}
                                        />
                                    </InputGroup>
                                    {errors.clubRange && touched.clubRange ? (
                                        <p className={"text-danger mb-0 error-form"}>{errors.clubRange}</p>
                                    ) : null}
                                </div>
                                <div className='mb-1 w-50'>
                                    ...
                                    <InputGroup>
                                        ...
                                        <Field type="number" className="form-control" id='shaftlength' placeholder='Shaft length' name="shaftLength"
                                            onChange={(e) => setFieldValue("shaftLength", e.target.value)}
                                        />
                                    </InputGroup>
                                    {errors.shaftLength && touched.shaftLength ? (
                                        <p className={"text-danger mb-0 error-form"}>{errors.shaftLength}</p>
                                    ) : null}
                                </div>
                                <div className='mb-1 w-50'>
                                    ...
                                    <ReactSelect className="react-select" classNamePrefix='select' options={shaftflex} id='shaftflex' placeholder='Select shaft flex' name="shaftFlex"
                                        onChange={(e) => setFieldValue("shaftFlex", e.value)}
                                    />
                                    {errors.shaftFlex && touched.shaftFlex ? (
                                        <p className={"text-danger mb-0 error-form"}>{errors.shaftFlex}</p>
                                    ) : null}
                                </div> */}
                <div className="w-100">
                  <div className="mb-1 w-50">
